...
export function MessageProvider(props) {
  ...
  function getMessages(id) {
    const page = getPage()
    api.get.messages(id, page).then(({ items, count, initialPost }) =>
      dispatch({
        type: 'SET_MESSAGES',
        messages: items,
        count,
  ...
  function postMessage(id, message, author, isPrivate) {
    const page = getPage()
    api.post.message(id, message, author, isPrivate).then(() => getMessages(id, page))
  }
