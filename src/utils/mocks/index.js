...
export function mockApi() {
  createServer({
    ...
    seeds(server) {
      ...
      const threads = fillAnArrayOfSortedDates(nbOfThreads, () =>
        faker.date.past()
      );
      threads.forEach((createdAt) => {
        const nbOfMessages = Math.round(Math.random() * 1000);
        const messages = fillAnArrayOfSortedDates(nbOfMessages, () =>
    ...
    routes() {
      this.get("/api/thread", (schema) => {
        ...
        return threads.models.map(
          ({ author, content, createdAt, messages, id, title }) => ({
            title,
            author,
            content,
            createdAt,
